...
def setup_data_loader(config, project_root):
    ...
    # Resolve mask directory paths relative to project_root if they are provided as relative paths
    train_mask_dir = None
    test_mask_dir = None

    if not mask_only:
        raw_train_mask_dir = config.get("train_binary_mask_dir")
        raw_test_mask_dir = config.get("test_binary_mask_dir")

        if raw_train_mask_dir:
            train_mask_dir = (
                raw_train_mask_dir
                if os.path.isabs(raw_train_mask_dir)
                else os.path.join(project_root, raw_train_mask_dir)
            )

        if raw_test_mask_dir:
            test_mask_dir = (
                raw_test_mask_dir
                if os.path.isabs(raw_test_mask_dir)
                else os.path.join(project_root, raw_test_mask_dir)
            )
